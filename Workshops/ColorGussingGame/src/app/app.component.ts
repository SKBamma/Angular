...
@Component({
  ...
  <button (click)="$display_history.set(!$display_history())">show/hide history</button>
  @if($display_history()){
  ...
  @for (history of $history(); track history; let odd =$odd) {
    ...
    <tr>
     ...
     <div id="color_space" [ngStyle]="{'background-color': $displayed_color()}">
      {{$displayed_color()}}
     </div>
  ...
  `,
  styles: [`#color_space {width:400px; height: 250px} button { margin: 20px; margin-left: 30px}`]
})
