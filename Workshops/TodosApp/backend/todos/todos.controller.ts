...
export const getTodos: RequestHandler<unknown, IResponse<ITodo[]>, BodyWithToken> =
    async (req, res, next) => {
        try {
...
export const addTodo: RequestHandler<unknown, IResponse<ITodo>, BodyWithToken & ITodo> =
    async (req, res, next) => {
        try {
            const { tokenData: { _id: user_id } } = req.body;
...
export const getTodoById: RequestHandler<{ todo_id: string; }, IResponse<ITodo>, BodyWithToken> =
    async (req, res, next) => {
        try {
            const { tokenData: { _id: user_id } } = req.body;
...
export const updateTodoById: RequestHandler<{ todo_id: string; }, IResponse<number>, BodyWithToken & ITodo>
    = async (req, res, next) => {
        try {
            const { tokenData: { _id: user_id } } = req.body;
            const { todo_id } = req.params;
...
export const deleteTodoById: RequestHandler<{ todo_id: string; }, IResponse<number>, BodyWithToken> =
    async (req, res, next) => {
        try {
            const { tokenData: { _id: user_id } } = req.body;
            const { todo_id } = req.params;
